...
export function App() {
  ...
  useEffect(() => {
    let timer: number;
    let shCmd = '"sed -i s/'.concat((isDarkModeEnabled) ? 'Light' : 'Dark').concat('/').concat((isDarkModeEnabled) ? 'Dark' : 'Light').concat('/g /home/workspace/.openvscode-server/data/Machine/settings.json || echo \'{\\"workbench.colorTheme\\": \\"Default Light Modern\\"}\' > /home/workspace/.openvscode-server/data/Machine/settings.json"')
    //console.log(shCmd);
    const start = async () => {
      setReady(() => false);
      ...
      await ddClient.docker.cli.exec("exec", [
        ...
        'mkdir',
        '-p',
        '/home/workspace/.openvscode-server/data/Machine/'
      ]);
      await ddClient.docker.cli.exec("exec", [
        '-d',
        'openvscode_embedded_dd_vm',
  ...
  return (
    <>
      ...
      {ready && (window.location.href = 'http://localhost:57081/')}
    </>
  );
}
